...
export const getCakesAndIngredients = async (req, res) => {
    try {
        const cakes = await Cake.find();
        
        const cakesWithIngredients = await Promise.all(
            cakes.map(async (cake) => {
                const ingredients = await RecipeIngredient.find({ cakeId: cake._id }).populate('ingredientId');
        ...
        res.status(200).json({ message: 'Cakes and ingredients retrieved successfully', data: cakesWithCosts });
...
export const createSignatureCake = async (req, res) => {
  try {
    ...
    const recipeIngredients = await RecipeIngredient.find({ cakeId: savedCake._id }).populate('ingredientId');
    res.status(201).json({ ...savedCake.toObject(), ingredients: recipeIngredients });
  } catch (error) {
...
export const updateSignatureCake = async (req, res) => {
  try {
    ...
    const recipeIngredients = await RecipeIngredient.find({ cakeId: req.params.id }).populate('ingredientId');
...
export const createCake = async (req, res) => {
  try {
    ...
    if (existingCake) {
      return res.status(400).json({ message: 'Cake with this name already exists' });
    }
    ...
    res.status(201).json({ ...cakeWithIngredients.toObject(), ingredients: recipeIngredients });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
...
export const createPackagingOption = async (req, res) => {
  try {
    ...
    if (existingOption) {
      return res.status(400).json({ message: 'Packaging option with this name already exists' });
    }
...
export const deletePackagingOption = async (req, res) => {
  try {
    ...
    const packagingOption = await PackagingOption.findByIdAndDelete(req.params.id);
    if (!packagingOption) {
      return res.status(404).json({ message: 'Packaging option not found' });
    }
